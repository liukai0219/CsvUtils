...
public class CsvListBeanWriter extends AbstractCsvWriter {
	private final MethodCache cache = new MethodCache();

	public void write(List<Object> data, String file, String... headers) throws IOException {
		if (data == null) {
			throw new NullPointerException("the data to write should not be null!");
		}
		if (file == null) {
	...
	public void write(List<Object> data, Writer writer, String... headers) throws IOException {
		if (data == null) {
	...
	private void writeContent(List<Object> data, String... headers) {
		...
		for (String header : headers) {
			...
			try {
				...
				throw new CsvUtilsReflectException(String.format("error extracting bean value for field %s", header),e);
			}
		}
		sb.replace(sb.length() - 1, sb.length(), "\r\n");
	}
